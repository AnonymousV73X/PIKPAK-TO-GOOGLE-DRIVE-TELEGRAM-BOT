...
def _find_local_api_bin() -> str:
    ...
    SKIP = {
        "proc","sys","dev","run","snap","boot","lib","lib64","lib32","usr",
...
def _probe_binary_flags(binary: str) -> dict:
    result = {
        "api_id_flag": None, "api_hash_flag": None,
...
def send_msg(chat_id, text, parse_mode="HTML", reply_markup=None, reply_to=None):
    ...
    def _try_send():
        try:
            ...
            if _using_local_api and ("connection refused" in err_str or "max retries" in err_str):
    ...
    def get_stats(self) -> dict:
        with self._lock:
            elapsed  = (str(datetime.now() - self.start_time).split(".")[0]
                        if self.start_time else "00:00:00")
...
class TransferManager:
    ...
    _MIN_SESSION_SECS = 60          # first session runs at least this long before watchdog fires
    ...
    def _edit(self, text, markup=None):
        if not self.status_msg_id:
            sent = send_msg(self.chat_id, str(text)[:4000], parse_mode="HTML", reply_markup=markup)
            if sent:
                self.status_msg_id = sent.message_id
                MsgStore.save(self.user_id, "status", self.chat_id, sent.message_id)
            return
        ok = edit_msg(self.chat_id, self.status_msg_id, str(text)[:4000],
    ...
    def render_status(self, phase="transferring") -> str:
        ...
        pct         = min(int(done_files / total_files * 100), 100) if total_files > 0 and done_files > 0 else 0
        ...
        spd_icon  = "🚀" if up_bps >= 10*1024*1024 else "⚡" if up_bps >= 3*1024*1024 else "🟡" if up_bps >= 512*1024 else "🔴"
    ...
    def _verify_remotes(self) -> bool:
        ...
        for remote in ["PIKKY", "GDRIVE"]:
            ...
            ok, _, err = self._run_cmd(f"{self.rclone_path} --config {self.config_file} lsd {remote}:")
            if not ok:
        ...
        if ok2:
            for line in out2.split("\n"):
                if line.startswith("Total size:"):
                    ...
                    self.total_size_str  = re.sub(r"\bByte\b", "Bytes",
    ...
    def _kill_port(self):
        try:
            subprocess.run(["fuser", "-k", str(self._WEBDAV_PORT) + "/tcp"],
    ...
    def _start_webdav(self) -> "subprocess.Popen | None":
        ...
        serve_cmd = [
            ...
            "--read-only", "--no-modtime", "--log-level", "ERROR",
        ]
        try:
            lfd  = open(log_path, "w")
            proc = subprocess.Popen(serve_cmd, stdout=lfd, stderr=lfd, start_new_session=True)
        except Exception as e:
        ...
        while time.monotonic() < deadline:
            ...
            if proc.poll() is not None:
                ...
                return None
            try:
                urllib.request.urlopen(f"http://127.0.0.1:{self._WEBDAV_PORT}", timeout=1)
                return proc
            except Exception:
                time.sleep(0.5)
    ...
    def _gdrive_list_done(self) -> set:
        try:
            r = subprocess.run(
                [self.rclone_path, "--config", self.config_file,
                 "lsf", f"GDRIVE:{self.destination_folder}", "--recursive", "--files-only"],
    ...
    def _start_copy_proc(self, filter_args: list):
        ...
        copy_cmd = [
            self.rclone_path, "--config", self.config_file,
            "copy", f"{webdav_remote}:", f"GDRIVE:{self.destination_folder}",
            "--transfers", "2", "--checkers", "1",
            "--size-only", "--drive-chunk-size", "64M",
            "--drive-acknowledge-abuse", "--drive-pacer-min-sleep", "200ms",
            "--drive-pacer-burst", "5", "--tpslimit", "3", "--tpslimit-burst", "5",
            "--buffer-size", "64M", "--use-mmap",
            "--retries", "1", "--low-level-retries", "3",
            "--timeout", "120s", "--contimeout", "30s", "--log-level", "INFO",
        ] + filter_args

        stderr_lines: list = []
        try:
            proc = subprocess.Popen(copy_cmd, stdout=subprocess.DEVNULL,
    ...
    def _fallback_rclone_copy(self, remaining: list) -> bool:
        ...
        copy_cmd    = [
            self.rclone_path, "--config", self.config_file,
            "copy", "PIKKY:", f"GDRIVE:{self.destination_folder}",
            "--transfers", "1", "--checkers", "1",
            "--size-only", "--drive-chunk-size", "32M",
            "--drive-acknowledge-abuse", "--drive-pacer-min-sleep", "500ms",
            "--drive-pacer-burst", "3", "--tpslimit", "2", "--tpslimit-burst", "3",
            "--buffer-size", "32M", "--retries", "3", "--low-level-retries", "5",
            "--timeout", "300s", "--contimeout", "60s", "--log-level", "INFO",
        ] + filter_args
        ...
        self.warn(f"Fallback copy finished with {len(still_remaining)} file(s) still missing (rc={rc})")
        return False
    ...
    def _do_transfer(self) -> bool:
        ...
        for session_idx in range(self._MAX_SESSIONS):
            ...
            remaining = [f for f in self.video_files
                         if f not in done_basenames and os.path.basename(f) not in done_basenames]

            self.ok(f"Session {session_idx + 1} done: "
            ...
            # ── Rate limit wait ───────────────────────────────────────────────
            is_rate_limit = any(k in err_tail for k in
                                ["ratelimitexceeded", "rate limit", "userratelimitexceeded"])
            if is_rate_limit:
                wait_secs = 90
            ...
            if reconnect_now or rc in (0, 5) or rc != 0:
                time.sleep(min(3 + session_idx * 2, 15))  # small backoff between sessions
                continue

        # ── FIX 4 + 5: MAX_SESSIONS hit → try fallback before giving up ──────
        if remaining:
            self.warn(f"Reached {self._MAX_SESSIONS} WebDAV sessions. Trying direct fallback…")
            self._edit(
            ...
            self.err(f"Fallback failed. {confirmed}/{len(self.video_files)} files on GDrive.")
            self._edit(
    ...
    def run(self):
        ...
        try:
            ...
            ns             = self.network_monitor.get_stats()
            transferred_size = (self.total_size_str if self.total_size_str not in ("Unknown","") else ns["total_uploaded"])
            elapsed_secs   = (datetime.now() - self.start_time).total_seconds() if self.start_time else 0
            active_secs    = max(elapsed_secs - 10, 1)
            avg_bps        = (self.total_size_bytes / active_secs if self.total_size_bytes > 0
            ...
            if self.stop_requested:
                ...
                rc_note  = (f"\n🔁  Reconnects  <b>{self.reconnect_count}</b>" if self.reconnect_count > 0 else "")
                self._edit(
                ...
                last_err  = next((l for l in reversed(self.logs) if "❌" in l or "⚠️" in l), "")
                self._edit(
            ...
            UserManager.save_transfer(
                ...
                self.destination_folder, len(self.video_files),
                self.total_size_str, transferred_size, speed, error_msg,
            )
...
def _home_text(user_id: int) -> str:
    ...
    api_badge   = (f"🟢 Local API active  ·  up to <b>{fmt_size(MAX_SEND_BYTES())}</b>"
                   if api_active else
                   f"🟡 Official API  ·  up to <b>{fmt_size(MAX_SEND_BYTES())}</b>  (→ /localapi for 2 GB)")
    return (
...
def cb_home_buttons(call: CallbackQuery):
    ...
    if action == "upload":
        ...
        threading.Thread(target=_drive_stats_inline, args=(cid, mid, user_id), daemon=True).start()
    elif action == "drivvy":
        threading.Thread(target=_drivvy_inline, args=(cid, mid, user_id), daemon=True).start()
    elif action == "pikky":
        threading.Thread(target=_pikky_inline, args=(cid, mid, user_id), daemon=True).start()
    elif action == "history":
        ...
        if mgr and mgr.transfer_in_progress:
            ...
            sent = send_msg(cid, mgr.render_status("transferring"), reply_markup=mgr._live_kb())
            if sent:
            ...
            edit_msg(cid, mid, "⚠️ No active transfer running.", reply_markup=_back_home_kb(user_id))
    elif action == "localapi":
        text, kb = _build_localapi_text_and_kb(user_id)
        edit_msg(cid, mid, text, reply_markup=_localapi_kb_with_back(user_id), parse_mode="HTML")
    elif action == "config":
        edit_msg(cid, mid,
                 ...
                 reply_markup=_back_home_kb(user_id), parse_mode="HTML")
    elif action == "back":
        # Return to home panel
        edit_msg(cid, mid, _home_text(user_id), reply_markup=_home_kb(user_id), parse_mode="HTML")
...
def cmd_guide(message: Message):
    ...
    sent = send_msg(message.chat.id, _guide_menu_text(), reply_markup=_guide_kb(user_id))
    if sent:
...
@bot.callback_query_handler(
    func=lambda c: (c.data.startswith("guide_") and
                    not c.data.startswith("guide_menu_")))
    ...
    try:
        if message.content_type == "text":
            ...
            with urllib.request.urlopen(
                f"https://api.telegram.org/file/bot{BOT_TOKEN}/{fp}", timeout=30) as resp:
...
def _do_upload(message_or_obj, user_id: int):
    ...
    sent = send_msg(chat_id, "🚀 <b>Transfer Starting…</b>\n\n" + DIVIDER_SM + "\n⏳ Initialising rclone…")
    if sent:
        mgr.status_msg_id = sent.message_id
...
def _do_pick(message_or_obj, user_id: int):
    ...
    threading.Thread(target=_pick_load_files, args=(chat_id, user_id, msg), daemon=True).start()


def _pick_load_files(chat_id: int, user_id: int, status_msg):
    rclone = get_rclone_path(user_id)
    cfg    = get_config_file(user_id)
    ok, out, err = run_rclone(rclone, cfg, "lsf", "PIKKY:", "--recursive", "--files-only", timeout=300)
    if not ok:
        if status_msg:
            edit_msg(chat_id, status_msg.message_id, f"❌ Could not list PikPak files:\n<code>{safe_escape(err)}</code>")
        return

    all_files = [f.strip() for f in out.split("\n") if f.strip()]
    if not all_files:
        if status_msg:
            edit_msg(chat_id, status_msg.message_id, "📭 PikPak is empty — no files found.")
        return
...
def _send_picker_page(chat_id: int, user_id: int, edit_mid=None):
    ...
    for i, fname in enumerate(page_files, start=start):
        ...
        kb.row(InlineKeyboardButton(f"{tick} {label}", callback_data=f"pick_tog_{user_id}_{i}"))

    # Navigation row
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("« Prev", callback_data=f"pick_page_{user_id}_{page-1}"))
    if end < total:
        nav.append(InlineKeyboardButton("Next »", callback_data=f"pick_page_{user_id}_{page+1}"))
    if nav:
        kb.row(*nav)

    if selected:
        kb.row(InlineKeyboardButton(f"🚀 Transfer {len(selected)} file(s)",
...
def cb_pick_go(call: CallbackQuery):
    ...
    mgr.destination_folder = f"{generate_alien_name()}-{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    managers[user_id] = mgr

    names_preview = "\n".join(f"  🎬 <code>{safe_escape(os.path.basename(f))}</code>"
                               for f in selected_files[:6])
    more = f"\n  <i>…and {len(selected_files)-6} more</i>" if len(selected_files) > 6 else ""
    sent = send_msg(
        chat_id,
    ...
    def _run_selected():
        ...
        error_msg                = None
        try:
            mgr.head(f"PikPak→GDrive (selected) | User {user_id} | {fmt_dt(mgr.start_time)}")
            ...
            try:
                ...
                pass
            UserManager.save_transfer(
                user_id, status, mgr.start_time, datetime.now(),
                mgr.destination_folder, len(selected_files),
                "Selected", "Unknown", "0 B/s", error_msg,
            )
...
def cmd_stop(message: Message):
    ...
    if not mgr or not mgr.transfer_in_progress:
        send_msg(message.chat.id, "⚠️ No active transfer.",
...
def cmd_status(message: Message):
    ...
    mgr     = managers.get(user_id)
    if not mgr or not mgr.transfer_in_progress:
        send_msg(message.chat.id, "⚠️ No active transfer running.",
                 reply_markup=_back_home_kb(user_id))
        return
...
def cb_refresh(call: CallbackQuery):
    ...
    try:
        bot.answer_callback_query(call.id, "✅ Refreshed!" if ok else "Already up to date.")
    except Exception:
        pass
...
def _build_localapi_text_and_kb(user_id: int):
    ...
    if _using_local_api:
        ...
        status = ("🟡 <b>Binary found, credentials saved</b>  ·  server not running."
                  if api_id and api_hash else
                  "🔴 <b>Binary found, but no credentials saved.</b>")
    else:
...
def cb_localapi_back(call: CallbackQuery):
    ...
    edit_msg(call.message.chat.id, call.message.message_id, text,
             reply_markup=_localapi_kb_with_back(user_id), parse_mode="HTML")
...
def _localapi_dlbin_receive(message: Message, user_id: int):
    ...
    if message.content_type == "document":
        ...
        try:
            ...
            ok  = _download_binary(
                ...
                dest, status.message_id if status else 0, chat_id)
        except Exception as e:
            ok = False
            print(f"[dlbin upload] {e}")
        _finish_binary_download(chat_id, user_id, status.message_id if status else 0, ok, dest)
        return
    ...
    threading.Thread(
        target=lambda: _finish_binary_download(
            chat_id, user_id, status.message_id if status else 0,
            _download_binary(url, dest, status.message_id if status else 0, chat_id), dest,
        ),
        daemon=True,
    ).start()
...
def cb_set_sendas(call: CallbackQuery):
    ...
    has_home = any(
        ...
        for row in (call.message.reply_markup.keyboard if call.message.reply_markup else [])
        for btn in row
    )
    if has_home:
        kb = _make_kb([
            (f"{'🟢 ' if value=='video'    else ''}🎬 Video",    f"set_sendas_{user_id}_video"),
            (f"{'🟢 ' if value=='document' else ''}📄 Document", f"set_sendas_{user_id}_document"),
            ("🔙 Go Back", f"home_back_{user_id}"),
        ])
    else:
        kb = _make_kb([
    ...
    try:
        ...
        pass
    try:
        bot.answer_callback_query(call.id, f"🟢 {'🎬 Video' if value=='video' else '📄 Document'}")
    except Exception:
        pass
...
def _drive_stats_inline(chat_id: int, mid: int, user_id: int):
    ...
    if not os.path.exists(rclone):
        edit_msg(chat_id, mid, "❌ rclone not ready. Use /config first.",
                 reply_markup=_back_home_kb(user_id))
        return
    ok, out, err = run_rclone(rclone, cfg, "about", "GDRIVE:")
    if not ok:
        edit_msg(chat_id, mid, f"❌ Failed: {safe_escape(err)}",
                 reply_markup=_back_home_kb(user_id))
        return
    total = used = free = trash = "Unknown"
    for line in out.split("\n"):
...
def _drivvy_inline(chat_id: int, mid: int, user_id: int):
    ...
    if not files:
        edit_msg(chat_id, mid, "📁 GDrive is empty.", reply_markup=_back_home_kb(user_id))
        return
    ...
    for line in file_lines:
        if cur_len + len(line) + 1 > 3600:
            pages.append(cur); cur = line; cur_len = len(line)
        else:
            cur = (cur + "\n" + line) if cur else line; cur_len += len(line) + 1
    if cur: pages.append(cur)
    for i, body in enumerate(pages):
        header = f"📁 <b>GDrive Files</b>  ({i+1}/{len(pages)})  <i>{total} total</i>\n\n"
        if i == 0:
            edit_msg(chat_id, mid, header + body,
                     reply_markup=_back_home_kb(user_id), parse_mode="HTML")
        else:
            send_msg(chat_id, header + body, reply_markup=(_back_home_kb(user_id) if i == len(pages)-1 else None))
        time.sleep(0.4)
...
def _pikky_inline(chat_id: int, mid: int, user_id: int):
    ...
    if ok2:
        ...
        if videos:
            ...
            kb        = _make_kb([("🗂 Pick & Transfer", f"home_pick_{user_id}"), ("🔙 Go Back", f"home_back_{user_id}")])
        else:
    ...
    edit_msg(chat_id, mid, (storage_text + video_text)[:4000],
             reply_markup=kb, parse_mode="HTML")
...
def cmd_drive(message: Message):
    ...
    if loading:
        threading.Thread(target=_drive_stats_inline,
                         args=(message.chat.id, loading.message_id, user_id), daemon=True).start()
...
def cmd_drivvy(message: Message):
    ...
    if loading:
        threading.Thread(target=_drivvy_inline,
                         args=(message.chat.id, loading.message_id, user_id), daemon=True).start()
...
def cmd_pikky(message: Message):
    ...
    if loading:
        threading.Thread(target=_pikky_inline,
                         args=(message.chat.id, loading.message_id, user_id), daemon=True).start()
...
def _build_sendfiles_text_and_kb(user_id: int):
    ...
    completed = [t for t in transfers if t["status"] in ("completed", "partial")
                 and t["destination_folder"]]
    if not completed:
        return None, None
    send_as   = UserManager.get_settings(user_id).get("send_as", "video")
...
def cb_senddir(call: CallbackQuery):
    ...
    if not os.path.exists(rclone) or not os.path.exists(cfg):
        edit_msg(cid, mid, "❌ rclone not ready. Run /upload first.",
                 reply_markup=_back_home_kb(user_id))
        return
    ok, out, err = run_rclone(rclone, cfg, "lsf", f"GDRIVE:{folder}",
                               "--recursive", "--files-only", timeout=120)
    if not ok:
        edit_msg(cid, mid, f"❌ Failed to list: {safe_escape(err)}",
                 reply_markup=_back_home_kb(user_id))
        return
    files = [f for f in out.split("\n") if f.strip()]
    if not files:
        edit_msg(cid, mid, "📁 No files found in this folder.",
                 reply_markup=_back_home_kb(user_id))
        return
...
def _generate_thumbnail(video_path: str, thumb_path: str) -> bool:
    ...
    for seek in unique_c:
        try:
            r = subprocess.run(
                ["ffmpeg", "-y", "-ss", _secs_to_ts(seek), "-i", video_path,
...
def _send_files_worker(chat_id, user_id, folder, files, rclone, cfg, send_as, status_msg_id):
    ...
    for i, filename in enumerate(files, 1):
        ...
        def _parse_size_to_bytes(s: str) -> int:
            ...
            if not m: return 0
            val, unit = float(m.group(1)), m.group(2).upper().replace("IB","").replace("B","")
    ...
    outcome     = "All files sent successfully." if all_ok else (
                  "Completed with some issues." if has_failed or has_skipped else "Done.")
...
def set_commands():
    try:
        bot.set_my_commands([
            ...
            telebot.types.BotCommand("upload",    "🚀 Start PikPak → GDrive (all videos)"),
...
def main():
    ...
    try:
        bot.infinity_polling(
            timeout=60, long_polling_timeout=30,
            skip_pending=True,
